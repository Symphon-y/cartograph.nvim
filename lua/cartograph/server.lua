-- cartograph.server — the local bridge: a minimal vim.uv HTTP/SSE server.
--
-- Thin transport only. It accepts loopback connections, hands raw bytes to
-- http_wire for parsing, gates data endpoints on a per-session token, serves
-- the bundled web UI, streams graph updates via the sse hub, and routes browser
-- POSTs through protocol to the injected engine. All wire-format, SSE and
-- routing *logic* lives in the pure modules this composes, which are unit-tested.

local M = {}

local uv = vim.uv or vim.loop
local wire = require('cartograph.http_wire')
local sse = require('cartograph.sse')
local protocol = require('cartograph.protocol')

-- Locate the bundled web/ directory (this file is lua/cartograph/server.lua).
local function web_dir()
  local src = debug.getinfo(1, 'S').source:sub(2)
  return vim.fn.fnamemodify(src, ':h:h:h') .. '/web'
end
...
-- Route a request. Returns the sse client wrapper when it upgraded the socket
-- to a long-lived SSE stream (so the caller keeps it open), nil otherwise.
...
function Server:on_connection()
  ...
  -- Per-connection state lives here (uv handles are userdata and can't hold
  -- arbitrary fields). `sse_ref` is set once a socket becomes an SSE stream.
  local buf = ''
  local handled = false
  local sse_ref = nil
  ...
  client:read_start(function(err, chunk)
    ...
    -- Wait for the full body on writes with a Content-Length.
...
-- Returns the server object (with :url(), :broadcast(), :stop()).
function M.start(opts)
  ...
  if not ok then
    self:stop()
    error('cartograph: could not start server: ' .. tostring(err))
  end
