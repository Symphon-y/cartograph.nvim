...
describe('cartograph.http_wire', function()
  describe('parse_request', function()
    it('parses method, path, query and headers', function()
      local raw = table.concat({
        'GET /api/graph?root=Foo%2FBar&depth=2 HTTP/1.1',
        'Host: 127.0.0.1',
        'X-Token: abc123',
        '',
        '',
      ...
      assert.are.equal('/api/graph', req.path)
      assert.are.equal('Foo/Bar', req.query.root) -- url-decoded
      assert.are.equal('2', req.query.depth)
      assert.are.equal('127.0.0.1', req.headers.host) -- header names lowercased
      assert.are.equal('abc123', req.headers['x-token'])
    end)

    it('parses a POST body', function()
      local body = '{"action":"expand","nodeId":"a:1:2"}'
      local raw = table.concat({
  ...
  describe('build_response', function()
    ...
    it('uses the right reason phrase for known statuses', function()
      assert.is_truthy(wire.build_response({ status = 403 }):match('^HTTP/1%.1 403 Forbidden'))
      assert.is_truthy(wire.build_response({ status = 404 }):match('^HTTP/1%.1 404 Not Found'))
    end)
  end)
